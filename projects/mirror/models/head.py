...
class DynamicHead(nn.Module):
    ...
    def forward(self, features, init_bboxes, init_features):
        ...
        init_features = init_features.unsqueeze(0).repeat(1, bs, 1)
        proposal_features = init_features.clone()

        # for rcnn_head in self.head_series:
        n = len(self.head_series)
        ...
        for i in range(n):
            ...
            container, class_logits, pred_bboxes, proposal_features = rcnn_head(features, container)

            if self.return_intermediate:
                cls_logits, pred_bbox = container['class_logits'], container['bboxes']
                inter_class_logits.append(cls_logits)
                inter_pred_bboxes.append(pred_bbox)
            
            container['bboxes'] = container['bboxes'].detach()
...
def build_dynamic_head(cfg, roi_input_shape):

    return DynamicHead(cfg, roi_input_shape)
...
class RCNNHead(nn.Module):
    ...
    def ffn_forward(self, obj_features):
    ...
    def forward(self, features, container):
        ...
        """
        fz = True
        # bboxes, pro_features, pooler = {'bboxes': bboxes, 'proposal_features': proposal_features, 'pooler': self.box_pooler}
        bboxes, pro_features, pooler = container['bboxes'], container['pro_features'], container['pooler']
        level = container['lid']
        ...
        gather_mask = alive_mask = torch.zeros(N, nr_boxes, 1).to(class_logits.device)
        ...
        tmp = {'class_logits': class_logits, 'bboxes': pred_bboxes, 'pro_features': obj_features}
            #     'cur_all_mask': cur_all_mask,
            #    'mask': mask, 'next_mask': next_mask,'gather_mask': gather_mask,
            #    'alive_mask': alive_mask}
        
        container.update(tmp)

        return container, class_logits, pred_bboxes, obj_features
