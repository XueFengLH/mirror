...
from utils.box_ops import box_cxcywh_to_xyxy, generalized_box_iou, box_ioa,box_iou_kk,box_iou
...
def build_matcher(cfg, cost_class, cost_bbox, cost_giou, use_focal=True):

    return HungarianMatcher(cfg, cost_class, cost_bbox, cost_giou, use_focal)


class HungarianMatcher_1(nn.Module):
    """This class computes an assignment between the targets and the predictions of the network
    ...
    @torch.no_grad()
    def forward(self, fz_bbox, out_bbox):
        """ Performs the matching
        ...
        # Compute the giou cost betwen boxes
        bs = 1
        num_queries = len(fz_bbox)

        cost_bbox = torch.cdist(fz_bbox[:,:4], out_bbox[:,:4], p=1)
        # cost_giou = -generalized_box_iou(fz_bbox[:,:4], out_bbox[:,:4])
        cost_kiou = box_iou_kk(fz_bbox[:,:4], out_bbox[:,:4])
        cost_class = torch.ger(fz_bbox[:,4],out_bbox[:,4])
        cost_iou,_ = box_iou(fz_bbox[:,:4],out_bbox[:,:4])
        # Final cost matrix
        C = cost_bbox
        # print("**********************************************************************************************************")
        # print(out_bbox)
...
def build_matcher_1(cfg, cost_class, cost_bbox, cost_giou, use_focal=True):
    return HungarianMatcher_1(cfg, cost_class, cost_bbox, cost_giou, use_focal)
